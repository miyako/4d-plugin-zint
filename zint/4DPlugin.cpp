...
void ZINT(sLONG_PTR *pResult, PackagePtr pParams)
{
	...
	inType.fromParamAtIndex(pParams, 3);
	inOptionName.fromParamAtIndex(pParams, 4);
	inOptionValue.fromParamAtIndex(pParams, 5);
	...
	if(sym)
	{
		...
		sym->symbology = inType.getIntValue();
		
		if(inOptionName.getSize() == inOptionValue.getSize())
		{
			for(unsigned int i = 0; i < inOptionName.getSize(); ++i)
			{
				option = inOptionName.getIntValueAtIndex(i);
				...
				switch (option) {
					...
					case ZINT_HEIGHT:
						{
							int height = (int)atoi((const char *)value.c_str());
						...
						break;
					case ZINT_WHITE_SPACE:
						{
							int whitespace = (int)atoi((const char *)value.c_str());
					...
					case ZINT_BORDER:
						{
							int border = (int)atoi((const char *)value.c_str());
						...
						break;
					case ZINT_BOX:
						if(atoi((const char *)value.c_str()))
					...
					case ZINT_DPI:
					{
						int _dpi = (int)atoi((const char *)value.c_str());
						if (_dpi > 72)
						{
							dpi = _dpi;
						}
					...
					case ZINT_SCALE:
						{
							float scale = (float)atof((const char *)value.c_str());
							if(scale >= 0.01)
							{
								sym->scale = scale;
							}
					...
					case ZINT_ROTATE:
						{
							int rotate = (int)atoi((const char *)value.c_str());
							switch(rotate)
							{
					...
					case ZINT_COLUMNS:
					{
						int columns = (int)atoi((const char *)value.c_str());
					...
					case ZINT_VERSION:
					{
						int version = (int)atoi((const char *)value.c_str());
					...
					case ZINT_SECURE:
					{
						int secure = (int)atoi((const char *)value.c_str());
					...
					case ZINT_PRIMARY:
						if(value.length() <= 90)
						{
							strcpy(sym->primary, (const char *)value.c_str());
						}
						break;
					case ZINT_MODE:
						{
							int mode = (int)atoi((const char *)value.c_str());
							if((mode >= 0) && (mode <= 6))
							{
								sym->option_1 = mode;
							}
						...
						break;
					case ZINT_NO_BACKGROUND:
							no_background = atoi((const char *)value.c_str());
						break;
					case ZINT_GS1:
						if(atoi((const char *)value.c_str()))
		...
		if(!ZBarcode_Encode(sym, s, n))
		{
			...
			switch(inOutputFormat.getIntValue())
			{
				case ZINT_OUTPUT_SVG:
					toSVG(sym, dpi, rotate_angle, no_background, outData, pResult);
					break;
				case ZINT_OUTPUT_PNG:
					toPNG(sym, dpi, rotate_angle, no_background, outData, pResult);
					break;
			}
	...
	outData.toParamAtIndex(pParams, 6);
...
void toPNG(zint_symbol *symbol, int dpi, int rotate_angle, bool no_background, C_TEXT &_dump, sLONG_PTR *pResult)
{
...
void toSVG(zint_symbol *symbol, int dpi, int rotate_angle, bool no_background, C_TEXT &_dump, sLONG_PTR *pResult)
{
